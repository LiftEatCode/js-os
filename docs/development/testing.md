...
- `src/command-center/activity/format.test.ts` (event/source labels including `goal.progress_updated`, filters, metadata JSON)
...
- `src/business-commands/goals.test.ts` (atomic Goal+event commands, progress, status lifecycle, no-ops, other-org, rollback; no Neon)
- `src/business-commands/work-items.test.ts` (atomic WorkItem+event commands, startedAt/completedAt, CANCELLED, parent cycles, no-ops, other-org, rollback; no Neon)
...
- `src/command-center/format.test.ts` (enum display labels)
