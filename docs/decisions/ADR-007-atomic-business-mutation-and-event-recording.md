...
- Residual concurrency: commands load current state inside the transaction. ToolRequest / ToolExecution status changes (Milestone 3.4) and Approval decisions (Milestone 3.5) use guarded `where({ id, status: from }).update(...)` so two callers cannot both win the same transition without detection. Attempt-number allocation retries the whole command on unique violation.
- Tool request/execution lifecycle (Milestone 3.4) uses the same `runBusinessCommand` / `commitStateAndEvent` boundary. Meaningful `tool.*` events commit with the state mutation. `requestToolUse` lands on the routed status in one transaction (no visible intermediate `REQUESTED`). ALWAYS tools also create the PENDING Approval and `approval.requested` in that transaction (Milestone 3.5). Owner approval/rejection/cancellation of a linked request updates Approval and ToolRequest together with both domain events. Execution success/failure updates ToolExecution and ToolRequest together.
